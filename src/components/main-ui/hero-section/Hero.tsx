...
function CameraDrift() {
  ...
  useFrame((_, dt) => {
    mouse.current.x = THREE.MathUtils.lerp(mouse.current.x, mouse.current.targetX, dt * 3)
    mouse.current.y = THREE.MathUtils.lerp(mouse.current.y, mouse.current.targetY, dt * 3)

    const targetX = mouse.current.x * 0.35
    const targetY = -mouse.current.y * 0.2
    camera.position.x = THREE.MathUtils.lerp(camera.position.x, targetX, dt * 1.5)
    camera.position.y = THREE.MathUtils.lerp(camera.position.y, targetY + 0.3, dt * 1.5)
    camera.lookAt(0, 0, 0)
  })
  return null
}
...
function CinematicModel({
  ...
  useFrame((_, dt) => {
    ...
    modelRef.current.rotation.y = THREE.MathUtils.lerp(modelRef.current.rotation.y, targetRotY, cDt * 2)
    modelRef.current.position.z = THREE.MathUtils.lerp(modelRef.current.position.z, targetZ, cDt * 1.5)
    modelRef.current.position.y = THREE.MathUtils.lerp(modelRef.current.position.y, targetY, cDt * 2)
    modelRef.current.rotation.z = THREE.MathUtils.lerp(modelRef.current.rotation.z, targetTilt, cDt * 1.8)

    const baseScale = isMobile ? 0.9 : 1.25
    const scaleBoost = 1 + scroll.velocity * 0.4
    targetScale.current = baseScale * scaleBoost
    const s = THREE.MathUtils.lerp(modelRef.current.scale.x, targetScale.current, cDt * 4)
...
function Section({ align, tag, headline, body }: SectionProps) {
  ...
  useEffect(() => {
    ...
    gsap.set(el.querySelector('.stat-bar-fill'), { scaleX: 0, transformOrigin: 'left' })
  ...
  return (
    ...
    >
      ...
      <div className={`flex flex-wrap gap-x-4 gap-y-1 ${isLeft ? '' : 'justify-end'} mb-6 max-w-lg`}>
        {headline.map((word, i) => (
      ...
      <div className={`flex flex-col gap-2 w-full max-w-xs ${isLeft ? '' : 'items-end'}`}>
        <div className="w-full h-px bg-[#CCC5B9]/10 relative overflow-hidden rounded-full">
          <div className="stat-bar-fill absolute inset-0 bg-gradient-to-r from-[#CCC5B9]/60 to-[#CCC5B9]/20 rounded-full" />
        </div>
      </div>

      <div
        className={`absolute top-1/2 ${isLeft ? 'left-0' : 'right-0'} w-px h-32 -translate-y-1/2`}
        style={{ background: 'linear-gradient(to bottom, transparent, rgba(204,197,185,0.3), transparent)' }}
      />
    </div>
  )
}
...
function LandingScreen() {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      gsap.fromTo(
        '.land-line',
        { y: '100%' },
        { y: '0%', stagger: 0.12, duration: 1.1, ease: 'power4.out', delay: 0.3 }
      )
      gsap.fromTo(
        '.land-sub',
        { opacity: 0 },
  ...
  return (
    <div ref={ref} className="h-screen flex flex-col items-center justify-start pb-32 relative z-10 select-none">
  ...
  useEffect(() => {
    ...
    const updateScroll = () => {
      ...
      if (canvasWrapRef.current) {
        ...
        canvasWrapRef.current.style.visibility = clamped <= 0 ? 'hidden' : 'visible'
      }
      ...
      >
        {canvasVisible && (
          ...
          >
            ...
            <directionalLight position={[8, 10, 5]} intensity={2.2} color="#ffffff" />
            <spotLight position={[-8, -6, -8]} intensity={6} color="#CCC5B9" angle={0.4} />
